...
class LearningExtractionImpl implements LearningExtractor {
  ...
  async extract(conversationId: string): Promise<Learning[]> {
    ...
    const textsToEmbed = learnings.map(
      (l) =>
  ...
  async search(
    ...
    if (options?.learningType) {
      learnings = learnings.filter(
        (l) => l.learningType === options.learningType
      );
    }
...
function displaySummaryLearning(learning: Learning): void {
  ...
  const abstraction =
    learning.abstraction.principle || learning.abstraction.pattern;
...
function displayDetailedLearning(learning: Learning): void {
  ...
  console.log(
    `  Can teach it: ${learning.understanding.canTeachIt ? "Yes" : "No"}`
  );
