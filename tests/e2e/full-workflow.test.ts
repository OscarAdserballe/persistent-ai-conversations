...
describe("Full Workflow E2E", () => {
  ...
  it("should persist data across database reopening", () => {
    ...
    const initialConvCount = db
      .prepare("SELECT COUNT(*) as count FROM conversations")
      .get() as { count: number };
    const initialMsgCount = db
    ...
    const afterConvCount = db
      .prepare("SELECT COUNT(*) as count FROM conversations")
      .get() as { count: number };
    const afterMsgCount = db
      .prepare("SELECT COUNT(*) as count FROM messages")
      .get() as { count: number };
    db.close();
  ...
  it("should verify FTS5 sync after ingestion", () => {
    ...
    const convCount = db
      .prepare("SELECT COUNT(*) as count FROM conversations")
      .get() as { count: number };
    const convFtsCount = db
      .prepare("SELECT COUNT(*) as count FROM conversations_fts")
      .get() as { count: number };
