...
describe('Ingest Command E2E', () => {
  ...
  beforeEach(() => {
    ...
    const testConfig = {
      ...
      ingestion: {
        batchSize: 10,
        progressLogging: false // Disable for cleaner test output
      }
    }
  ...
  it('should handle invalid JSON in conversations file', () => {
    const invalidJsonPath = join(__dirname, '../tmp/invalid-conversations.json')
  ...
  it('should support custom platform option', () => {
    ...
    const db = new Database(testDbPath)
    const conversations = db.prepare('SELECT platform FROM conversations').all()
  ...
  it('should batch process messages correctly', () => {
    ...
    const smallBatchConfigPath = join(__dirname, '../tmp/small-batch-config.json')
    writeFileSync(smallBatchConfigPath, JSON.stringify(smallBatchConfig, null, 2))
  ...
  it('should preserve conversation order and indexes', () => {
    ...
    const byConversation = new Map<string, Array<{ conversation_index: number }>>()
    for (const msg of messages) {
      if (!byConversation.has(msg.conversation_uuid)) {
        byConversation.set(msg.conversation_uuid, [])
      }
      byConversation.get(msg.conversation_uuid)!.push({ conversation_index: msg.conversation_index })
    }
  ...
  it('should handle re-running ingest on existing database', () => {
    ...
    const db = new Database(testDbPath)
    const firstCount = db.prepare('SELECT COUNT(*) as count FROM messages').get() as { count: number }
    db.close()
    ...
    try {
      ...
      const secondCount = db2.prepare('SELECT COUNT(*) as count FROM messages').get() as { count: number }
